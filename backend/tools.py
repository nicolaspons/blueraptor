...
def save_perf_stats(
    ...
    """
    Prints and save some performance metrics of the strategy.
    ...
    if bootstrap:
        perf_func = pd.timeseries.perf_stats_bootstrap
    else:
        perf_func = pd.timeseries.perf_stats
    ...
    try:
        filename = os.path.join(path, ticker + ".csv")
        perf_stats.to_csv(filename, columns=["Backtest"])
    except FileNotFoundError:
        print(
            "Can't save the dataframe, wrong ticker : {} or path {}.".format(
                ticker, path
            )
        )
